...
% Choosing the values of "lambda" that I will use. It must be under 0.5
lambda_values = [0.01 0.05 0.25 0.49];

% Changing "lambda".
for i = 1 : length(lambda_values)
    
    %  Fill the calculations arrays of our functions
    [calling_f1(i),~,~] = bisectionDerivative(func1, lambda_values(i), -1, 3);
    [calling_f2(i),~,~] = bisectionDerivative(func2, lambda_values(i), -1, 3);
    [calling_f3(i),~,~] = bisectionDerivative(func3, lambda_values(i), -1, 3);
...
hold on;
plot(calling_f1);
title(sprintf('f1 Calculations: Bisection with Derivative'));
...
hold on;
plot(calling_f2);
title(sprintf('f2 Calculations: Bisection with Derivative'));
...
hold on;
plot(calling_f3);
title(sprintf('f3 Calculations: Bisection with Derivative'));
...
for i = 1 : length(lambda_values)
    ...
    [~, alphas1, betas1] = bisectionDerivative(func1, lambda_values(i), -1, 3);
    [~, alphas2, betas2] = bisectionDerivative(func2, lambda_values(i), -1, 3);
    [~, alphas3, betas3] = bisectionDerivative(func3, lambda_values(i), -1, 3);
    ...
    figure(plot_number);
    plot(betas1);
    title(sprintf('f1 edges of range for lambda = %f: Bisection with Derivative', lambda_values(i)));
    ...
    figure(plot_number);
    plot(betas2);
    title(sprintf('f2 edges of range for lambda = %f: Bisection with Derivative', lambda_values(i)));
    ...
    figure(plot_number);
    plot(betas3);
    title(sprintf('f3 edges of range for lambda = %f: Bisection with Derivative', lambda_values(i)));
    ylabel(sprintf('f3 edges of range'));
    xlabel('lambda');
    plot_number = plot_number + 1;

    fprintf("For bisectionDerivative a-b is %f\n",alphas1(end)-betas1(end));
